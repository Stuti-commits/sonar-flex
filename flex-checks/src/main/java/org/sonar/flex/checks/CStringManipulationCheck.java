...
public class CStringManipulationCheck extends FlexCheck {
    ...
    public List<AstNodeType> subscribedTo() {
        return Arrays.asList(
                FlexGrammar.C_STRING_LITERAL,
                FlexGrammar.C_CHAR_CONSTANT,
                FlexGrammar.C_DECLARATION,
                FlexGrammar.C_PRIMARY_EXPR);
    }

    @Override
    public void visitNode(AstNode node) {
        if (node.is(FlexGrammar.C_STRING_LITERAL)) {
            checkStringLiteral(node);
        } else if (node.is(FlexGrammar.C_CHAR_CONSTANT)) {
            checkCharConstant(node);
        } else if (node.is(FlexGrammar.C_DECLARATION)) {
            checkCharArraySize(node);
        } else if (node.is(FlexGrammar.C_PRIMARY_EXPR)) {
    ...
    private void checkCharArraySize(AstNode decl) {
        ...
        AstNode specifiers = decl.getFirstChild(FlexGrammar.C_DECLARATION_SPECIFIERS);
        if (specifiers == null)
            return;
        ...
        AstNode initDeclList = decl.getFirstChild(FlexGrammar.C_INIT_DECLARATOR_LIST);
        if (initDeclList == null)
            return;

        for (AstNode initDecl : initDeclList.getChildren(FlexGrammar.C_INIT_DECLARATOR)) {
            // C_INIT_DECLARATOR: C_DECLARATOR = C_INITIALIZER
            AstNode declarator = initDecl.getFirstChild(FlexGrammar.C_DECLARATOR);
            AstNode initializer = initDecl.getFirstChild(FlexGrammar.C_INITIALIZER);
            ...
            AstNode strLit = initializer.getFirstDescendant(FlexGrammar.C_STRING_LITERAL);
            if (strLit == null)
                continue;
    ...
    private void checkAdjacentStrings(AstNode primaryExpr) {
        ...
        for (AstNode child : children) {
            if (child.is(FlexGrammar.C_STRING_LITERAL)) {
                if (prevStr != null) {
                    addIssue(
    ...
    private static boolean isCharType(AstNode specifiers) {
        ...
        if (text == null) {
            ...
            for (AstNode child : specifiers.getDescendants(FlexGrammar.C_TYPE_SPECIFIER)) {
    ...
    private static int extractArraySize(AstNode declarator) {
        AstNode directDecl = declarator.getFirstChild(FlexGrammar.C_DIRECT_DECLARATOR);
        if (directDecl == null)
            return -1;

        // C_DIRECT_DECLARATOR: C_IDENTIFIER [ C_EXPRESSION? ]
        AstNode bracket = directDecl.getFirstChild(FlexGrammar.C_EXPRESSION);
        if (bracket == null)
            return -1;
