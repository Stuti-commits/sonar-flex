...
public class CFunctionDeclarationCheck extends FlexCheck {
    ...
    public List<AstNodeType> subscribedTo() {
        return Collections.singletonList(FlexGrammar.C_FUNCTION_DEFINITION);
    }
    ...
    public void visitNode(AstNode funcDef) {
        ...
        AstNode specifiers = funcDef.getFirstChild(FlexGrammar.C_DECLARATION_SPECIFIERS);
        AstNode declarator = funcDef.getFirstChild(FlexGrammar.C_DECLARATOR);
        AstNode body = funcDef.getFirstChild(FlexGrammar.C_FUNCTION_BODY);
    ...
    private void checkInvalidStorageClass(AstNode specifiers) {
        for (AstNode child : specifiers.getDescendants(FlexGrammar.C_STORAGE_CLASS_SPECIFIER)) {
    ...
    private void checkFunctionName(AstNode declarator) {
        ...
        AstNode nameNode = directDecl.getFirstChild(FlexGrammar.C_IDENTIFIER);
        if (nameNode == null)
            return;
    ...
    private void checkEmptyBody(AstNode body, AstNode funcDef) {
        ...
        AstNode blockItemList = body.getFirstChild(FlexGrammar.C_BLOCK_ITEM_LIST);
